...
export class FluxKontextModel extends BaseModel {
  ...
  validateInput(input: any): FluxKontextInput {
    ...
    if (inputImage) {
      ...
      console.log("✨ Text-based image editing mode enabled");
    } else {
    ...
    console.log("⚡ Using FLUX Kontext - Standard text-based editing with consistent quality");
    return validated;
  }

  transformInput(input: FluxKontextInput): Record<string, any> {
    console.log("\n🔄 FLUX-KONTEXT INPUT TRANSFORMATION:");
    ...
    if (input.input_image) {
      ...
      console.log("🖼️ Including input image for text-guided editing");
      console.log("🎯 Model excels at: style transfer, object changes, consistent editing");
    }
    ...
    console.log("📝 FLUX Kontext: Reliable text-guided image editing and generation");
    console.log("⚖️ Balanced performance and cost-effectiveness");
    return transformed;
  }
}